...
支持 **Claude Code 和 Codex**，并提供 **OpenClaw / Hermes 实验性适配**；支持**中英文(安装时选定并锁定,也可选按消息自动切换)**、即时开场提示、决策优先的结果播报、多 Agent 音色区分；一条命令安装、跨平台播放、离线秒回。
...
- **结果播报**：这一轮结束时，Codex 会在本地选择一句简洁的结论、失败原因或**需要你拍板的选择（决策优先）**。你听到就能直接回复、继续推进，把“单向播报”变成“双向对话”；需要精确措辞时，也可以用显式语音标记覆盖本地选择。

Codex 默认在本地从最终回复中选择完整的结论、失败原因或待确认事项，不再要求模型每轮生成额外标记。需要精确控制播报时，仍可使用隐藏标记 `<!-- voice: ... -->` 覆盖本地摘要。
如果既没有隐藏标记，也提取不到可用摘要，hook 会播一句安全兜底「已完成。」并记录 `no-marker-fallback`，避免完全没声音。
...
| 结果语音播报 | Agent 完成后默认播本地结果摘要；需要精确控制时可用隐藏标记覆盖。 |
...
Agent 会根据当前客户端选择安装方式，检查 Node.js、Python、Edge TTS
和音频播放器依赖，注册可用的 hooks，并验证 Skill 能被加载和播放测试音。
如果当前 Codex 版本需要单独批准 hook，Agent 会明确告诉你还需要批准哪一项。
...
Codex 的结果播报默认来自本地整句选择器，不增加模型 Token。隐藏标记 `<!-- voice: ... -->` 只作为可选覆盖；旧格式 `<<voice: ...>>` 仍然兼容。
...
| Agent 答完 | hook 本地选择结果摘要，或读取隐藏 `<!-- voice: … -->` 覆盖 | 真正的结论；缺摘要则短兜底 |
...
- 结果播报：优先播可用结果摘要，隐藏标记可精确覆盖；
- 无可用摘要：播短兜底并记录 `no-marker-fallback`。
- Codex 后台记忆维护任务：默认静音；只有存在显式语音标记时才播报（`suppressMaintenance` 默认为 `true`）。
