...
Codex 对新安装的 Hook 要求一次性安全确认。安装结束后，在 Codex 中输入 `/hooks`，分别批准 `UserPromptSubmit` 和 `Stop`，然后新建任务测试。安装器会同时通过终端和语音提醒，`doctor.mjs` 也会分别显示两项授权状态。

默认安装到 `~/.agents/skills/codex-voice-reply`。如需换目录，可设置 `VOICE_REPLY_INSTALL_DIR` 后再运行安装命令。
